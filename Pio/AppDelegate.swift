...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self,
            ...
            name: UserManager.userDidLogoutNotification,
            object: nil)
    ...
    func application(application: UIApplication, openURL url: NSURL, sourceApplication: String?, annotation: AnyObject) -> Bool {
        print("Deeplink \(url.absoluteString) from \(sourceApplication)")
        if url.scheme == "pio" {
            UserManager.singleton.continueLoginFlow(url)
            return true
        }
        return false
    }
    ...
    func userDidLogout() {
        let tweetsVC = storyBoard.instantiateInitialViewController()
        window?.rootViewController = tweetsVC
    }
